...
def generate_code_code_adjacent(pids, patient_admission, admission_codes_encoded, medicine_codes_encoded, code_num, threshold=0.01, med_code_num = 12):
    print('generating code code adjacent matrix ...')
    n = code_num
    m = med_code_num
    adj = np.zeros((n, n), dtype=int)
    adj_med = np.zeros((m, m), dtype=int)
    for i, pid in enumerate(pids):
        ...
        for admission in patient_admission[pid]:
            ...
            med_codes = medicine_codes_encoded[admission[EHRParser.adm_id_col]]
            ...
            for med_row in range(len(med_codes) - 1):
                for med_col in range(med_row + 1, len(med_codes)):
                    med_c_i = med_codes[med_row]
                    med_c_j = med_codes[med_col]
                    adj_med[med_c_i, med_c_j] += 1
                    adj_med[med_c_j, med_c_i] += 1
    ...
    med_norm_adj = normalize_adj(adj_med)
    mede_a = med_norm_adj < threshold
    mede_b = adj_med.sum(axis=-1, keepdims=True) > (1 / threshold)
    adj_med[np.logical_and(mede_a, mede_b)] = 0

    return adj, med_norm_adj
