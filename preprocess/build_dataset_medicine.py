...
def split_patients(patient_admission, admission_codes, code_map, train_num, valid_num, seed=6669):
    np.random.seed(seed)
    common_pids = set()
    ...
    valid_num = len(patient_admission) - train_num
    ...
    return train_pids, valid_pids


def build_code_xy(pids, patient_admission, admission_codes_encoded, max_admission_num, code_num, medicine_codes_encoded, medicine_code_num):
    n = len(pids),
    if type(n) == tuple:
        n = n[0]
    x = np.zeros((n, max_admission_num, code_num), dtype=bool)
    m = np.zeros((n, max_admission_num, medicine_code_num), dtype=bool)
    ...
    for i, pid in enumerate(pids):
        ...
        for k, admission in enumerate(admissions[:-1]):
            medicine_codes = medicine_codes_encoded[admission[EHRParser.adm_id_col]]
            m[i, k, medicine_codes] = 1
