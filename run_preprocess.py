...
if __name__ == '__main__':
    ...
    #################以上的代码都是读数据与统计数据#############################
    '''
        code_map将每个code映射为一个索引 OrderDict([('414.01',0),('411.1',1),...])
        admission_codes_encoded 为一个字典 将admission_codes中的codes变成映射的索引 
        原本是OrderDict([(172335,['414.01','411.1','582.81']),(...)])变成OrderDict([(172335,['0','1','210'])])
    '''
    ...
    '''
    train_pids, valid_pids = split_patients(
        patient_admission=patient_admission,
        admission_codes=admission_codes,
        code_map=code_map,
        train_num=conf[dataset]['train_num'],
    ...
    print('There are %d train, %d valid, '  % (len(train_pids), len(valid_pids)))
    #构建一个global combination graph 遍历所有患者，只要存在疾病共现则连接一条边
    ...
    # train_code_x:(pid,就诊次数,疾病); train_codes_y:(pid,疾病) 待预测疾病; train_visit_lens 历史就诊次数
    ...
    #train_code_x:(len_train_pids,max_admission_num,code_num) train_visit_lens:len_train_pids
    train_neighbors = generate_neighbors(train_code_x, train_visit_lens, code_adj) #得到患者code的邻居
    ...
    ##得到persistent diseases、Emerging neighbors、Emerging unrelated diseases分别在第三个维度的0，1，2的位置
