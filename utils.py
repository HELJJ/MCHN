...
def med_load_adj(path, device=torch.device('cpu')):
    filename = os.path.join(path, 'med_code_adj.npz')
    adj = torch.from_numpy(load_sparse(filename)).to(device=device, dtype=torch.float32)
    return adj
class EHRDataset:
    def __init__(self, data_path, label='m', batch_size=32, shuffle=True, device=torch.device('cpu')):
        super().__init__()
        self.path = data_path
        self.code_x, self.visit_lens, self.y, self.divided, self.neighbors, self.med_codes= self._load(label)
    ...
    def _load(self, label):
        ...
        med_codes = load_sparse(os.path.join(self.path, 'med_code.npz'))
        return code_x, visit_lens, y, divided, neighbors, med_codes
    ...
    def __getitem__(self, index):
        ...
        code_x = torch.from_numpy(self.code_x[slices]).to(device) ##(pid,就诊次数,疾病) #根据这个对患者每一次就诊构建超图，顶点和边以及顶点的特征
        # 构建三个输入：一个是(pid,就诊次数,顶点/疾病) #一个是(pid,就诊次数,边)
        ...
        med_codes = torch.from_numpy(self.med_codes[slices]).to(device)
        return code_x, visit_lens, divided, y, neighbors, med_codes
